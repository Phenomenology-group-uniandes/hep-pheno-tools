...
class ElectronParticle(Particle):
    ''' A class representing an electron particle initialized from the Electron
    Leaf in Delphes files.

    Attributes:
        tlv (TLorentzVector): A TLorentzVector object that defines the
        electron's momentum and energy.
    ...
    Example:
        To create an ElectronParticle object using data from a delphes file
        event:
    ...
    def __init__(self, event, j):
        ''' Initializes an ElectronParticle object using data extracted from a
        delphes file event.

        Args:
            event (ROOT.TTree): A ROOT.TTree object containing the event data.
            j (int): An integer representing the index of the electron particle
            in the event.
