...
def read_excel(path, index_colum=0):
    ...
    - index_colum: column of the table that contains the indexes of the
    DataFrame (by default it is column 0).
...
def smooth(Data, log=False):
    ...
    Parameters:
    - Data*: DataFrame with the data, each entry refers to the value of a
    variable based on the column and row values. For example, if we consider
    two independent variables A, B and a function F(A,B), Data is a matrix
    where the rows take the values of A and the columns the values of B, i.e.
    each entry in the matrix will be just F(A,B).
    ...
    # In order to apply griddata (scipy function that allows interpolation)
    # it is necessary to rewrite the data of a matrix in a three-column format.
    ...
    # At this point, the data can be easily read, now what must be done is to
    # create the values ​​of x that we want to interpolate, that is,
    # the continuous intervals that we want to interpolate, I will call these
    # "x" and "y". However, the case in which one variable is fixed and only
    ...
    if (len(columns) == 1):
        ...
        data_i.index = x
        data_i.columns = y
        # griddata always returns in the form of a column vector, but if in
        # this case and it is fixed, our data must be in a row vector format,
        # that does the .T, it takes out the transpose.
        data_i = data_i.T
        data_i.columns = [round(i, 2) for i in data_i.columns]
        data_i.index = [round(i, 2) for i in data_i.index]
    else:
    ...
    data_i.sort_index(level=0, ascending=False, inplace=True)
    # Allows the graph to show the y-axis growing upwards
...
def plot_heatmap(
    ...
    Parameters:
    - level_curves: Directory containing the level curves to be ploted. It must
    have the structure {value (float): label(string),...}.
    - level_curves_labels_locations: List with suggested coordinates
    [(x1,y1), (x2,y2),...] of the positions where you want the signs of each
    level curve to be: The order does not matter.
    - zoom_region: Directory containing the positions to be zoom_regioned. It
    must have the structure {x1 (string): value (float), x2 (string):
    value (float), y1 (string): value (float), y2 (string): value (float),}.
    - **kwargs: Optional parameters like title, title_right, title_left,
    x_label, y_label, cbar_label, color, File_name, etc.
    ...
    # Depending on the shape of the data, the curves are plotted differently.
    # In the event that one of the two independent variables is fixed, then
